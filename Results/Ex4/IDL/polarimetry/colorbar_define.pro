;
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
;come inserire i commmenti sui plot, es distinguere curve 
;con colori differenti...
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

; xyouts , 140,100,'Coh. BEFORE Filtering',/device, charsize=1.5 , color= 250.$
; & xyouts , 140,80, 'Coh. AFTER Filtering',/device, color=150., charsize=1.5

;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
;                  INSERIMENTO COLORBAR
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
;ricordati di allargare la finestra di 200 pixel, piu´ o meno...altrimenti si sovrappone all´immagine.
; 
;   
; loadct,13
; window, k, xsize=sz(1)/scalex+200, ysize=sz(2)/scaley, TITLE = "PstoPv ratio"
; TV,BYTSCL(rebin (data,sz(1)/scalex, sz(2)/scaley),0.,2.)
; k=k+1
;     colorbar = Obj_New("COLORBAR", Title='Ps/Pv', Format='(F8.2)',/vertical)
;     colorbar->Draw
;     colorbar->SetProperty, Range=[0., 2.], /Erase, /Draw
...
struct = { COLORBAR, $     ; The COLORBAR object class definition.
   ...
   range:LonArr(2), $      ; The colorbar data range. Default is [0,255].
